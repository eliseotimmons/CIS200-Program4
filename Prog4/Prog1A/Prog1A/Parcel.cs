...
public abstract class Parcel : IComparable<Parcel> //called method comparer w/ class
{
    ...
    //pre: none
    //Post: correct handling for null values with test
    public int CompareTo(Parcel test)
    {
        if (test == null)
            return 1;


        if (this.CalcCost().CompareTo(test.CalcCost()) == 0)
            return 0;

        return this.CalcCost().CompareTo(test.CalcCost());
